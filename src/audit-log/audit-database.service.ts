...
export class AuditDatabaseService {
    ...
    public async disableAll() {
        const results = await this.sequelize.query<{ TABLE_NAME: string }>(this.sqlAuditedTables(), {
    ...
    private async getSchema(tableName: string, selectedFields: string[]): Promise<InformationSchemaType> {
        const results = await this.sequelize.query<{ TABLE_NAME: string, COLUMN_NAME: string, COLUMN_KEY: string }>(this.sqlSchemaInformation(), {
            type: QueryTypes.SELECT,
        ...
        results.forEach((result) => {
            ...
            selectedFields.length == 0 || selectedFields.includes(result.COLUMN_NAME.toUpperCase())
    ...
    private async disbleAudit(tableName: string) {
        this.sequelize.query(`DROP TRIGGER IF EXISTS audit_log_delete_${tableName};`);
        this.sequelize.query(`DROP TRIGGER IF EXISTS audit_log_create_${tableName};`);
        this.sequelize.query(`DROP TRIGGER IF EXISTS audit_log_update_${tableName};`);
    }
